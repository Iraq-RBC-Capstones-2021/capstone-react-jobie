export default {
  about: {
    HTitle: "ابحث عن وظيفتك",
    HSubTitle: "عش حلمك",

    AboutUs1: "عنا",
    AboutUs2: "نحن?",
    AboutUsDescription:
      "مهمتنا هي بناء جسر بين المهنيين وأرباب العمل الذين يحتاجون إلى وظيفة والذين يحتاجون إلى شخص ما للقيام بهذه المهمة نيابة عنهم ، هذه المنصة تجعل الاتصالات أسهل وأسرع ، في مجتمع ينمو فيه قطاع التكنولوجيا بسرعة الطلب على التكنولوجيا المهنيين ينموون كذلك نحن نسعى جاهدين لوضع الباحثين عن عمل في المقام الأول ومنحهم حرية الوصول للبحث عن وظائف وسير ذاتية وشركات بحثية. نربط كل يوم ملايين الأشخاص بفرص جديدة. ولكن هذا بالطبع يسمح أيضًا للشركات بالعثور على المرشح المناسب بشكل أسرع ، مما يسمح للباحثين عن عمل برؤية وظائفهم المفتوحة.",

    HowItWorks: "كيف تعمل",
    CreateResume: "أنشئ سيرتك الذاتية",
    CreateResumeDescription:
      "أنشئ سيرتك الذاتية وأضف مهاراتك وخبراتك إلى ملفك الشخصي. يمكنك أيضًا إضافة خبرتك التعليمية والعملية إلى ملفك الشخصي",
    ApplyForJobs: "التقدم للوظائف",
    ApplyForJobsDescription:
      "ابحث عن وظائف وتقدم لها. يمكنك أيضًا البحث عن الشركات والتقدم لوظائفها.",
    GetHired: "احصل على وظيفة",
    GetHiredDescription:
      "بمجرد تقديمك لوظيفة ، سيتم إخطارك عندما يقوم صاحب العمل بالرد على طلبك.",

    WhyChooseUs1: "لماذا الاختيار",
    WhyChooseUs2: "نحن?",
    Posted: "تم النشر",
    PostedDescription: "15 ألف وظيفة شاغرة",
    Saved: "تم الحفظ",
    SavedDescription: "5k لوظائف الشاغرة",
    Submitted: "مقدم",
    SubmittedDescription: "12k السيرة الذاتية",
    Hired: "استأجرت",
    HiredDescription: "18k الموظفين",
    MeetOurTeam1: "تلبية لدينا",
    MeetOurTeam2: "فريق",

    Zahraa: "زهراء يعرب",
    ZahraRole: "مطور أول | مصمم",

    Wisam: "وسام ناجي",
    WisamRole: "مطور أول | مصمم",

    Rebaz: "ريباز فاريد",
    RebazRole: "مدير المشروع | مطور",

    Aya: "ايه عبدالسلام محمد",
    AyaRole: "المطور | مترجم",

    Bnar: "بنار كمال ",
    BnarRole: "المطور | مصمم",

    Lara: "لارا رؤوف",
    LaraRole: "المطور | مصمم",

    OurCollaborators1: "لنا ",
    OurCollaborators2: "المتعاونون",
  },

  contact: {
    GetInTouch: "ابقى على تواصل",
    FirstName: "الاسم الأول",
    LastName: "الكنية",
    Email: "بريد الالكتروني",
    Message: "رسالة",
    Send: "يرسل",
    emailHolder: "بريدك الالكتروني",
    messageHolder: "اكتب رسالتك هنا...",
    ContactUs: "اتصل بنا",
    Location: "السليمانية ، العراق",
    Phone: "+964 750 113 0495",
    EmailUs: "Jobie@contact.com",
    OpenHours: "9:00 - 17:00",
  },

  home: {
    HTitle1: "10,254 وظائف",
    HTitle2: "مدرجة هنا!",
    HSubTitle: "ابحث عن وظيفة أحلامك الآن",
    Search: "بحث",
    TypeToSearch: "اكتب للبحث",

    FullTimeJobs: "وظائف بدوام كامل",
    FullTimeJobsDescription:
      "إذا كنت قد بدأت في حياتك المهنية وترغب في العمل بدوام كامل ، فإن وظيفة بدوام كامل هي الوظيفة المناسبة لك فقط ، وسيظهر لك النقر هنا جميع الوظائف الحالية بدوام كامل في منصتنا ",
    ViewAllFullTimeJobs: "عرض جميع الوظائف بدوام كامل",

    PartTimeJobs: "وظائف بدوام جزئي",
    PartTimeJobsDescription:
      "إذا كنت قد بدأت في حياتك المهنية وترغب في العمل بدوام جزئي ، فإن الوظيفة بدوام جزئي هي الوظيفة المناسبة لك ، سيؤدي النقر هنا إلى إظهار جميع الوظائف الحالية بدوام جزئي في منصتنا",
    ViewAllPartTimeJobs: "عرض جميع الوظائف بدوام جزئي",

    RemotesJobs: "وظائف التحكم عن بعد",
    RemotesJobsDescription:
      "إذا تم تعيينك في حياتك المهنية وترغب في العمل عن بُعد ، فإن وظيفة التحكم عن بُعد هي الوظيفة المناسبة لك ، وسيظهر لك النقر هنا جميع وظائف أجهزة التحكم عن بُعد الحالية في نظامنا الأساسي",
    ViewAllRemotesJobs: "عرض جميع وظائف أجهزة التحكم عن بعد",

    Categories: "استكشف حسب الفئات",

    Design: "تصميم",
    DesignNumbers: "67 وظيفة شاغرة",

    Frontend: "مطور الواجهه نهاية المقدمة",
    FrontendNumbers: "180 وظيفة شاغرة",

    Backend: "مطور الواجهه الخلفيه",
    BackendNumbers: "223 وظيفة شاغرة",

    WebDeveloper: "مطور ويب",
    WebDeveloperNumbers: "445 وظيفة شاغرة",

    Network: "شبكة الاتصال",
    NetworkNumbers: "321 وظيفة شاغرة",

    ProjectManager: "مدير المشروع",
    ProjectManagerNumbers: "635 وظيفة شاغرة",

    DataAnalyzer: "تحليلات البيانات",
    DataAnalyzerNumbers: "342 وظيفة شاغرة",

    ViewAllCategories: "عرض جميع الفئات",

    LatestJobs: "أحدث الوظائف",
    Position: "موقع",
    Salary: "مرتب",
    Date: "تاريخ",
    Action: "تطبيق",
    Apply: "تطبيق",
    FullTime: "وقت كامل",
    PartTime: "دوام جزئى",
    Remote: "بعيد",

    OurCustomers: "عملائنا",
    OurCustomersDescription:
      "لقد ساعدنا 2568 مستخدمًا في العثور على الوظيفة التي يحلمون بها.",
    Rebaz: "ريباز فاريد",
    RebazRole: "مطور ويب @ Scopic",
    RebazFeedback:
      "لقد كنت أعمل مع Jobie منذ فترة وأنا سعيد حقًا بالخدمة. لقد وجدت أنه من السهل جدًا العثور على وظيفة وأنا سعيد حقًا لأنني عثرت عليها.",

    Zahraa: "زهراء يعرب",
    ZahraRole: "مطور أول @ Acklen Avenue",
    ZahraFeedback:
      "لقد كنت أعمل مع Jobie منذ فترة وأنا سعيد حقًا بالخدمة. لقد وجدت أنه من السهل جدًا العثور على وظيفة وأنا سعيد حقًا لأنني عثرت عليها.",

    Lara: "لارا رؤوف",
    LaraRole: "DevOps @ Google",
    LaraFeedback:
      "لقد كنت أعمل مع Jobie منذ فترة وأنا سعيد حقًا بالخدمة. لقد وجدت أنه من السهل جدًا العثور على وظيفة وأنا سعيد حقًا لأنني عثرت عليها.",

    UsingOurServices: "أكثر من 20000 شركة تستخدم خدمتنا لتوظيف موظفيها.",
  },

  roadmap: {
    Htitle: "خرائط طريق المطور",
    HSubTitle: "اتبع خرائط الطريق هذه وكن بطلاً في أي مسار تختاره!",

    WebDevelopment: "تطوير الشبكة",
    Frontend: "مطور الواجهه نهاية المقدمة",
    Backend: "همطور الواجهه الخلفيه",
    FullStack: "كومة كاملة",

    Design: "تصميم",
    UIUX: "UI/UX",
    Graphic: "الرسومات",
    Logo: "شعار",

    NetworkSecurity: "الشبكة والأمن",
    Network: "شبكة الاتصال",
    Security: "حماية",
    Cloud: "سحاب",
  },

  jobs: {
    JobsFider: "مكتشف الوظائف",
    JobsFinerDesc: "ابحث عن أفضل وظيفة لك هنا",
    Search: "بحث",

    Filter: "منقي",
    SalaryRanges: "نطاق الراتب",
    From: "من عند",
    To: "إلى",
    Location: "موقع",
    TypeOfEmployment: "نوع العمل",
    Category: "فئة",
    Experience: "خبرة",
    WrokLevel: "مستوى العمل",
    ChooseAllThatApplies: "اختر كل ما ينطبق",
    Total: "المجموع",
    Results: "نتائج",
    SortBy: "ترتيب حسب: الأحدث",
  },

  navbar: {
    Home: "الصفحة الرئيسية",
    About: "عنا",
    Contact: "اتصل",
    Jobs: "وظائف",
    Roadmap: "خريطة الطريق",
    Search: "بحث",
    Login: "تسجيل الدخول",
    Account: "الحساب",
    EditProfile: "تعديل الملف الشخصي",
    ViewProfile: "عرض الملف الشخصي",
    PostJob: "انشاء وظيفة",
    Logout: "تسجيل الخروج",
  },

  footer: {
    JobieDesc:
      "جوبی هي عبارة عن منصة يمكن أن تساعدك في إعداد حياتك المهنية ، فنحن نقدم خدمات من اكتشاف المسار الصحيح للتقدم في حياتك المهنية من خلال خارطة طريق أو منصب مناسب من أفضل الشركات",

    Navigate: "التنقل",
    Home: "الصفحة الرئيسية",
    About: "معلومات عنا",
    Contact: "اتصل بنا",
    Jobs: "وظائف",
    Roadmap: "خريطة الطريق",

    Jobs: "وظائف",
    WebDeveloper: "مطور ويب",
    GraphicDesign: "تصميم غرافيك",
    Translator: "مترجم",
    BackendDeveloper: "مطور الواجهه الخلفيه ",
    UIUX: "مصمم UI UX",

    Services: "خدمات",
    BuildYourResume: "أنشئ سيرتك الذاتية",
    ListYourJobs: "قائمة الوظائف الخاصة بك",
    SearchJobs: "بحث وظائف",
    HireProfessionals: "توظيف المهنيين",
    RateAnEmployee: "قيم صاحب العمل",
  },

  JobView: {
    ViewAllJobs: "عرض جميع الوظائف",
    ApplyNow: "قدم الآن",
    Details: "تفاصيل",
    JobDescription: "المسمى الوظيفي",
    Responsibilities: "وتشمل مسؤوليات",
    Background: "الخلفية والخبرة",
    SimilarJobs: "وظائف مماثلة",
    edit: "تعديل",
    AlreadyAplly: "تطبق بالفعل",
    OfferedSalary: "الراتب المعروض",
    Gender: "جنس تذكير أو تأنيث",
    Experience: "خبرة",
    WorkLevel: "مستوى العمل",
    Category: "فئة",
  },

  ProfileView: {
    Hire: "توظيف",
    DownloadCV: "تحميل السيرة الذاتية",
    Bio: "لم تتم إضافة سيرة ذاتية بعد",
    Contact: "اتصل",
    Skills: "مهارات",
    WorkExperience: "خبرة في العمل",
    Education: "تعليم",
  },

  ProfileEdit: {
    EditProfile: "تعديل الملف الشخصي / ",
    SetUp: "قم بإعداد صفحة سيرتك الذاتية الشخصية",
    UploadPhoto: "حمل الصورة",
    Delete: "حذف",
    ImageDesc: "يجب أن تكون الصورة 400 × 400 بكسل على الأقل كملف png أو jpeg",

    ResumeFile: "استئناف الملف",
    Choose: "يختار",
    NoFileSelected: "لم يتم اختيار اي ملف",

    BasicInfo: "معلومات أساسية",
    FirstName: "الاسم الأول",
    LastName: "الكنية",
    Title: "العنوان",
    Biography: "سيرة شخصية",

    Contact: "اتصل",
    Location: "موقع",
    Phone: "هاتف",
    Email: "بريد الالكتروني",

    Social: "اجتماعي",

    Skills: "مهارات",
    SkillsDesc: "أضف المهارات كقيم مفصولة بفواصل.",

    AddItem: "اضافة عنصر",
    Remove: "يزيل",

    WorkExperience: "خبرة في العمل",
    ExperienceItem: "عنصر الخبرة ",
    Company: "شركة",
    Location: "موقع",
    TypeOfEmployment: "نوع العمل ",
    Position: "موقع",
    Date: "تاريخ",

    Education: "تعليم",
    EducationItem: "عنصر التعليم",
    UniversitySchool: "جامعة / مدرسة",
    Major: "رئيسي",
    Date: "تاريخ",

    Save: "يحفظ",
    Cancel: "يلغي",
  },
};
